...
def demo_maskrcnn(network, ctx, prefix, epoch,img_path,
    ...
    #print output
    rois = output[mod.output_names[0]].asnumpy()[:, 1:]
    ...
    boxes= pred_boxes
    #bus cyc motorcycle bigcar car ped crane
    #CLASSES  = ('__background__', 'bigcar', 'bus', 'car', 'crane', 'cyc', 'motorcycle', 'ped')
    CLASSES  = ('__background__', 'bus', 'cyc', 'motorcycle', 'bigcar', 'car', 'ped', 'crane')
    ...
    if vis:
        ...
        #plt.savefig("figures/" + img_path[:-4] + ".png")
        cv2.imwrite("figures/" + img_path[:-4] + ".jpg", im)
def parse_args():
...
def main():
    ...
    ctx = mx.gpu(args.gpu)
    print args
    imgs = os.listdir(args.image_name)
    for item in imgs:
        img = args.image_name + "/" + item
