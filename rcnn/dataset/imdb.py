...
class IMDB(object):
    ...
    def create_roidb_from_box_list(self, box_list, gt_roidb):
        ...
        for i in range(self.num_images):
            ...
            if gt_roidb is not None and gt_roidb[i]['boxes'].size > 0:
                ...
                gt_overlaps = bbox_overlaps(boxes.astype(np.float32, copy=False),
    ...
    def evaluate_recall(self, roidb, candidate_boxes=None, thresholds=None):
        ...
        for area_name, area_count in zip(area_names[1:], area_counts):
            print('percentage of %s %.3f' % (area_name, area_count / total_counts))
        print('average number of proposal %d' % (total_counts / self.num_images))
        for area_name, area_range in zip(area_names, area_ranges):
            ...
            for i in range(self.num_images):
                ...
                max_gt_overlaps = roidb[i]['max_overlaps']
                ...
                overlaps = bbox_overlaps(boxes.astype(np.float32, copy=False), gt_boxes.astype(np.float32, copy=False))
