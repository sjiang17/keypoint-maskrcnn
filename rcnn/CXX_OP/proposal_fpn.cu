...
class ProposalFPNGPUOp : public Operator{
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    std::vector<Tensor<xpu, 4>> score_vec;
    score_vec.push_back(in_data[proposal_fpn::kClsProbStride64].get<xpu, 4, float>(s)); // batch_idx, 2 * anchor_idx, height_idx, width_idx
    score_vec.push_back(in_data[proposal_fpn::kClsProbStride32].get<xpu, 4, float>(s)); // batch_idx, 2 * anchor_idx, height_idx, width_idx
    score_vec.push_back(in_data[proposal_fpn::kClsProbStride16].get<xpu, 4, float>(s)); // batch_idx, 2 * anchor_idx, height_idx, width_idx
    score_vec.push_back(in_data[proposal_fpn::kClsProbStride8].get<xpu, 4, float>(s)); // batch_idx, 2 * anchor_idx, height_idx, width_idx
    score_vec.push_back(in_data[proposal_fpn::kClsProbStride4].get<xpu, 4, float>(s)); // batch_idx, 2 * anchor_idx, height_idx, width_idx
    
    std::vector<Tensor<xpu, 4>> bbox_delta_vec;
    bbox_delta_vec.push_back(in_data[proposal_fpn::kBBoxPredStride64].get<xpu, 4, float>(s)); // batch_idx, 4 * anchor_idx, height_idx, width_idx
    bbox_delta_vec.push_back(in_data[proposal_fpn::kBBoxPredStride32].get<xpu, 4, float>(s)); // batch_idx, 4 * anchor_idx, height_idx, width_idx
    bbox_delta_vec.push_back(in_data[proposal_fpn::kBBoxPredStride16].get<xpu, 4, float>(s)); // batch_idx, 4 * anchor_idx, height_idx, width_idx
    bbox_delta_vec.push_back(in_data[proposal_fpn::kBBoxPredStride8].get<xpu, 4, float>(s)); // batch_idx, 4 * anchor_idx, height_idx, width_idx
    bbox_delta_vec.push_back(in_data[proposal_fpn::kBBoxPredStride4].get<xpu, 4, float>(s)); // batch_idx, 4 * anchor_idx, height_idx, width_idx

    Tensor<xpu, 2> im_info = in_data[proposal_fpn::kImInfo].get<xpu, 2, float>(s); // batch_idx, 3(height, width, scale)

    Tensor<xpu, 3> out = out_data[proposal_fpn::kOut].get<xpu, 3, float>(s); // batch_idx, rois_idx, 5(batch_idx, x1, y1, x2, y2), batch_idx is needed after flatten
    Tensor<xpu, 3> out_score = out_data[proposal_fpn::kScore].get<xpu, 3, float>(s); // batch_idx, rois_idx, 1(score)

    int rpn_pre_nms_top_n = param_.rpn_pre_nms_top_n;
    ...
    Tensor<xpu, 1, uint8_t> workspace = ctx.requested[proposal_fpn::kTempSpace].get_space_typed<xpu, 1, uint8_t>(Shape1(WORKSPACE_LIMIT), s);
    uint64_t allocated_bytes = 0ULL;

    std::vector<int> strides = param_.feature_strides.info;
    std::vector<float> rpn_min_sizes = param_.rpn_min_sizes.info;
    std::vector<proposal_fpn::ProposalOpInputs> input_enums = {proposal_fpn::kClsProbStride64, proposal_fpn::kClsProbStride32, proposal_fpn::kClsProbStride16, proposal_fpn::kClsProbStride8, proposal_fpn::kClsProbStride4};
    
    int total_anchor_count = 0;
    std::vector<int> proposal_stride_offsets;    
    for (auto &&scores: score_vec) {
      proposal_stride_offsets.push_back(total_anchor_count);
      total_anchor_count += scores.size(1) / 2 * scores.size(2) * scores.size(3);
    }
    
    Tensor<xpu, 3> all_proposals(reinterpret_cast<float *>(workspace.dptr_ + allocated_bytes), Shape3(1, total_anchor_count, 5));
    allocated_bytes += total_anchor_count * 5 * sizeof(float);
    CHECK_LT(allocated_bytes, WORKSPACE_LIMIT) << "Allocating more memory than workspace limit";

    /* per stride */
    for (int i = 0; i < strides.size(); i++) {
      auto&& scores = score_vec[i];
      auto&& bbox_deltas = bbox_delta_vec[i];
      int nbatch = scores.size(0);
      CHECK_EQ(nbatch, 1) << "Proposal FPN supports bs=1 only";
      ...
      base_anchor[0] = 0.0;
      base_anchor[1] = 0.0;
      base_anchor[2] = strides[i] - 1.0;
      base_anchor[3] = strides[i] - 1.0;
      ...
      Tensor<xpu, 3> proposals(all_proposals.dptr_ + 5 * proposal_stride_offsets[i], Shape3(nbatch, count, 5));

      // prevent padded predictions
      int real_height = static_cast<int>(cpu_im_info[0] / strides[i]);
      int real_width = static_cast<int>(cpu_im_info[1] / strides[i]);
      ...
      FRCNN_CUDA_CHECK(cudaMemcpy(proposals.dptr_,
                                  anchors.data(), 
      ...
      // CAUTION!!! scores are two-way softmax instead of one-way sigmoid, you have to shift it to get the true scores.
      // UNDERSTAND THIS BEFORE DOING ANY MODIFICATION!!! scores.dptr_ + count
      ProposalGridKernel<<<dimGrid, dimBlock>>>(
        count, num_anchors, height, width, strides[i],
        scores.dptr_ + count, proposals.dptr_);
      FRCNN_CUDA_CHECK(cudaPeekAtLastError());
      ...
      if (param_.iou_loss) {
        IoUPredKernel<<<dimGrid, dimBlock>>>(
          ...
          proposals.dptr_, bbox_deltas.dptr_, proposals.dptr_);
      } else {
      ...
      FilterBoxKernel<<<dimGrid, dimBlock>>>(
        count, rpn_min_sizes[i] * cpu_im_info[2], proposals.dptr_);
      FRCNN_CUDA_CHECK(cudaPeekAtLastError());
    } /* end of per stride */

    /* copy score to a continuous memory */
    Tensor<xpu, 1> score(reinterpret_cast<float *>(workspace.dptr_ + allocated_bytes), Shape1(total_anchor_count));
    allocated_bytes += total_anchor_count * sizeof(float);
    CHECK_LT(allocated_bytes, WORKSPACE_LIMIT) << "Allocating more memory than workspace limit";

    Tensor<xpu, 1, int> order(reinterpret_cast<int *>(workspace.dptr_ + allocated_bytes), Shape1(total_anchor_count));
    allocated_bytes += total_anchor_count * sizeof(int);
    CHECK_LT(allocated_bytes, WORKSPACE_LIMIT) << "Allocating more memory than workspace limit";

    dim3 dimGrid((total_anchor_count + kMaxThreadsPerBlock - 1) / kMaxThreadsPerBlock);
    dim3 dimBlock(kMaxThreadsPerBlock);
    CheckLaunchParam(dimGrid, dimBlock, "CopyScore");
    CopyScoreKernel<<<dimGrid, dimBlock>>>(
      total_anchor_count, all_proposals.dptr_, score.dptr_, order.dptr_);
    FRCNN_CUDA_CHECK(cudaPeekAtLastError());
    ...
    ReorderProposalsKernel<<<dimGrid, dimBlock>>>(
      rpn_pre_nms_top_n, all_proposals.dptr_, order.dptr_, ordered_proposals.dptr_);
    FRCNN_CUDA_CHECK(cudaPeekAtLastError());
    ...
    Tensor<cpu, 1, uint64_t> mask_host_tensor = ctx.requested[proposal_fpn::kTempSpace].get_host_space_typed<1, uint64_t>(Shape1(boxes_num * col_blocks));
    ...
    PrepareOutput<<<dimGrid, dimBlock>>>(
      rpn_post_nms_top_n, ordered_proposals.dptr_, keep.dptr_, out_size, 0,
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    using namespace mshadow;
    using namespace mshadow::expr;
    CHECK_EQ(in_grad.size(), 11);

    Stream<xpu> *s = ctx.get_stream<xpu>();
    Tensor<xpu, 4> gscores64 = in_grad[proposal_fpn::kClsProbStride64].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gscores32 = in_grad[proposal_fpn::kClsProbStride32].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gscores16 = in_grad[proposal_fpn::kClsProbStride16].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gscores8 = in_grad[proposal_fpn::kClsProbStride8].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gscores4 = in_grad[proposal_fpn::kClsProbStride4].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gbbox64 = in_grad[proposal_fpn::kBBoxPredStride64].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gbbox32 = in_grad[proposal_fpn::kBBoxPredStride32].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gbbox16 = in_grad[proposal_fpn::kBBoxPredStride16].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gbbox8 = in_grad[proposal_fpn::kBBoxPredStride8].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gbbox4 = in_grad[proposal_fpn::kBBoxPredStride4].get<xpu, 4, real_t>(s);
    Tensor<xpu, 2> ginfo = in_grad[proposal_fpn::kImInfo].get<xpu, 2, real_t>(s);

    // can not assume the grad would be zero
    Assign(gscores64, req[proposal_fpn::kClsProbStride64], 0);
    Assign(gscores32, req[proposal_fpn::kClsProbStride32], 0);
    Assign(gscores16, req[proposal_fpn::kClsProbStride16], 0);
    Assign(gscores8, req[proposal_fpn::kClsProbStride8], 0);
    Assign(gscores4, req[proposal_fpn::kClsProbStride4], 0);
    Assign(gbbox64, req[proposal_fpn::kBBoxPredStride64], 0);
    Assign(gbbox32, req[proposal_fpn::kBBoxPredStride32], 0);
    Assign(gbbox16, req[proposal_fpn::kBBoxPredStride16], 0);
    Assign(gbbox8, req[proposal_fpn::kBBoxPredStride8], 0);
    Assign(gbbox4, req[proposal_fpn::kBBoxPredStride4], 0);
    Assign(ginfo, req[proposal_fpn::kImInfo], 0);
  }
...
template<>
Operator* CreateOp<gpu>(ProposalFPNParam param) {
  return new ProposalFPNGPUOp<gpu>(param);
}
}  // namespace op
}  // namespace mxnet
