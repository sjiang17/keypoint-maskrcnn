...
class ProposalFPNOp : public Operator{
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    // Shape<3> fg_scores_shape = Shape3(in_data[proposal_fpn::kClsProbStride].shape_[1] / 2,
    //                                   in_data[proposal_fpn::kClsProbStride].shape_[2],
    //                                   in_data[proposal_fpn::kClsProbStride].shape_[3]);

    // real_t* foreground_score_ptr = in_data[proposal_fpn::kClsProbStride].dptr<real_t>()
    //                                 + fg_scores_shape.Size();
    Tensor<xpu, 4> scores = in_data[proposal_fpn::kClsProbStride16].get<cpu, 4, real_t>(s);
    Tensor<cpu, 4> bbox_deltas = in_data[proposal_fpn::kBBoxPredStride16].get<cpu, 4, real_t>(s);
    Tensor<cpu, 2> im_info = in_data[proposal_fpn::kImInfo].get<cpu, 2, real_t>(s);

    Tensor<cpu, 3> out = out_data[proposal_fpn::kOut].get<cpu, 3, real_t>(s);
    Tensor<cpu, 3> out_score = out_data[proposal_fpn::kScore].get<cpu, 3, real_t>(s);
    ...
    Tensor<cpu, 1> workspace = ctx.requested[proposal_fpn::kTempSpace].get_space<cpu>(
    ...
    base_anchor[0] = 0.0;
    base_anchor[1] = 0.0;
    base_anchor[2] = param_.feature_strides.info[2] - 1.0;
    base_anchor[3] = param_.feature_strides.info[2] - 1.0;
      ...
      for (index_t i = 0; i < num_anchors; ++i) {
        for (index_t j = 0; j < height; ++j) {
          for (index_t k = 0; k < width; ++k) {
            ...
            workspace_proposals[n][index][0] = workspace_proposals[n][i][0] + k * param_.feature_strides.info[2];
            workspace_proposals[n][index][1] = workspace_proposals[n][i][1] + j * param_.feature_strides.info[2];
            workspace_proposals[n][index][2] = workspace_proposals[n][i][2] + k * param_.feature_strides.info[2];
            workspace_proposals[n][index][3] = workspace_proposals[n][i][3] + j * param_.feature_strides.info[2];
    ...
    int real_height = static_cast<int>(im_info[0][0] / param_.feature_strides.info[2]);
    int real_width = static_cast<int>(im_info[0][1] / param_.feature_strides.info[2]);
    ...
    utils::FilterBox(&workspace_proposals, param_.rpn_min_sizes.info[2] * im_info[0][2]);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Stream<xpu> *s = ctx.get_stream<xpu>();
    Tensor<xpu, 4> gscores = in_grad[proposal_fpn::kClsProbStride16].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gbbox = in_grad[proposal_fpn::kBBoxPredStride16].get<xpu, 4, real_t>(s);
    Tensor<xpu, 2> ginfo = in_grad[proposal_fpn::kImInfo].get<xpu, 2, real_t>(s);

    // can not assume the grad would be zero
    Assign(gscores, req[proposal_fpn::kClsProbStride16], 0);
    Assign(gbbox, req[proposal_fpn::kBBoxPredStride16], 0);
...
template<>
Operator *CreateOp<cpu>(ProposalFPNParam param) {
  return new ProposalFPNOp<cpu>(param);
}

Operator* ProposalFPNProp::CreateOperator(Context ctx) const {
