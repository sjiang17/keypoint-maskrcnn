...
class FPNROIPoolOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        area_threshold.update(
            {'stride%s' % self._feat_stride_fpn[-1]: [area_threshold['stride%s' % self._feat_stride_fpn[-1]][0], 0]})
        ...
        for s in self._feat_stride_fpn:
            ...
            if len(index) > 0:
                ...
                if config.ROIALIGN:
                    roi_pool = mx.nd.contrib.ROIAlign_v2(feat_dict['stride%s' % s], _rois, (self._pool_h, self._pool_w),
                                                         1.0 / float(s))
                else:
                    roi_pool = mx.nd.ROIPooling(feat_dict['stride%s' % s], _rois, (self._pool_h, self._pool_w),
                                                1.0 / float(s))
